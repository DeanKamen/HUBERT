...
{ //for when they both have the same size. 
	...
	for (unsigned d = 0; d < depA; d++)
	{
		...
		matMul_transposeB<rowsA, colsA, rowsB, colsB>(get_layer(A, rowsA, colsA, depA, d), rhs, get_layer(C, rowsA, rowsB, depA, d));
	}
}
...
{ //for when they both have the same size. 
	//We assume B is already "transposed"
	//assert(colsA == rowsB);
	for (unsigned d = 0; d < depA; d++)
	{
		matMul<rowsA, colsA, rowsB, colsB>(get_layer(A, rowsA, colsA, depA, d), get_layer(B, rowsB, colsB, depB, d), get_layer(C, rowsA, colsB, depA, d));
	}
}
#endif